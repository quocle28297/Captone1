...
function load_table_accept()
{
	if(isset($_POST['detail-room']))
	{
		...
		while($row = mysqli_fetch_array($result))
		{
			$name=get_name_from_id($row["CONTRACT_USERS_ID"]);
			$name_room=get_room_name_from_id($row["CONTRACT_ROOM_ID"]);
		...
		//$datediff = $timesend - $timesend;
			...
			<tr>
			<td><p class="table_trunganh">'.$name_room.'</p></td>
			<td><a href="viewprofile.php?profile='.$row["CONTRACT_USERS_ID"].'" target="_blank" ><p class="table_trunganh_id">'.$name.'<p></a></td>
...
{	
	...
	//định dạng d m y
		$timestar=date("d/m/Y", strtotime($timestar));
		$timestar1=date("m/d/Y", strtotime($timestar));
		$timesend=date("d/m/Y", strtotime($timesend));
		//$Room_price=number_format($row["ROOM_PRICE"]).'₫';
	...
	//form tiền
		...
		<span class="form-control">'.$name_room .'</span>
		</div>  
		</div>
		<br/> <br/><br/><br/><br/>
		<div class="col-xs-12 col-sm-4 col-md-4">      
		...
		date_s_e="'.$row["CONTRACT_STARTDATE"].'"
		date_e_e="'.$row["CONTRACT_ENDDATE"].'"
		gia_e="'.$Price_1.'"
...
//
// get name users fromid
function get_name_from_id($id)
{
	$connect = mysqli_connect("localhost", "root", "", "qlphongtro");  
	$query ="SELECT `USERS_NAME` FROM `users` WHERE `USERS_ID`= $id "; 
...
// get  room name
function get_room_name_from_id($id)
{
	$connect = mysqli_connect("localhost", "root", "", "qlphongtro");  
	$query ="SELECT `ROOM_NAME` FROM `room` WHERE `ROOM_ID`= $id "; 
