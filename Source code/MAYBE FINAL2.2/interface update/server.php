...
{
  echo"<script>";       
  echo  "setTimeout(function(){ alert('$text'); }, 3000);";
  echo "</script>"; 
}
...
if (isset($_POST['SignIn'])) {
  ...
  if (count($errors) == 0) {
    ...
    if (mysqli_num_rows($results) == 1) {
      ...
      array_push($errors, "Sai tài khoản /Mật khẩu");
     //header('Location: home-map.php?login-fail');
    }
  }
}
...
if (isset($_POST['save-zone'])) 
{
  ...
  if (count($errors) == 0) 
  {
    ...
    if($results)
    {
      ...
      header('Location:management-zone.php?zone_ok');
    }
    else
    {
...
//end create zone
//
if(isset($_GET['zone_ok']))
{
   array_push($statusMessage, "Bạn vừa thêm khu thành Công!!");
}
if(isset($_GET['delete-ok']))
{
   array_push($statusMessage, "Bạn vừa xóa khu thành công!!");
}
//
//edit zone
