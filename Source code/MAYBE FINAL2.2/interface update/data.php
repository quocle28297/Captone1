...
//get  f
		...
		// xoa phong
		//
		function delete_room_xoa($conn,$id_room)
		{
			echo $query="
			DELETE FROM `room` where ROOM_ID = '$id_room'
			";
			mysqli_query($conn,$query);
		}

		function delete_image_xoa($conn,$id_room)
		{
			echo $query="
			DELETE FROM `image` where IMAGE_ROOM_ID = '$id_room'
			";
			mysqli_query($conn,$query);
		}

		function delete_contract_xoa($conn,$id_room)
		{
			echo $query="
			DELETE FROM `contract` where CONTRACT_ROOM_ID = '$id_room'
			";
			mysqli_query($conn,$query);
		}

		function delete_zone_xoa($conn,$id_zone)
		{
			echo $query="
			DELETE FROM `zone` where ZONE_ID = '$id_zone'
			";
			mysqli_query($conn,$query);
		}
		//
		...
		{ 
			$id_room=$_POST["id_edit_room"];
			delete_image_xoa($conn,$id_room);	
			delete_contract_xoa($conn,$id_room);
			delete_room_xoa($conn,$id_room);
		}
		//
		//xóa khu
		if(isset($_POST["id_edit_zone"]))
		{
			$id_zone=$_POST["id_edit_zone"];
			$query = "SELECT * FROM room where ROOM_ZONE_ID= $id_zone ";
			$result=mysqli_query($conn,$query);
			while($row = mysqli_fetch_array($result)) 
			{
				$id_room=$row[0];
