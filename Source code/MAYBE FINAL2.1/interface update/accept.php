          ...
          <form id="insert_acc" method="post>
            <div class="row ">
              ...
              <div class="form-group ">
                ...
                <input name="date-s" id="date-s" type="date" class="form-control" min="<?php echo date('Y-m-d', strtotime('-10 years'));?>" max="<?php echo date('Y-m-d', strtotime('+10 years'));?>"/>
                <!-- <span class="form-error" id="Name-error-message">...</span> -->
              </div> 
              <div class="form-group ">
                <label for="Size">Ngày kết thúc: </label>
                <input name="date-e" id="date-e" type="date" class="form-control" min="<?php echo date('Y-m-d', strtotime('-10 years'));?>" max="<?php echo date('Y-m-d', strtotime('+10 years'));?>"/>
<!--                 <span class="form-error" id="error-message-date-s">...</span> -->
              </div> 
            </div>  
          </div>
        ...
        <div class="modal-body">
          <form id="insert_end" method="post>
            <div class="row ">
              ...
              <div class="form-group " id="tiem-s">
                <label for="Size">bắt đầu </label>
                <input name="date-s-e" id="date-s-e" type="date" class="form-control"  min="<?php echo date('Y-m-d', strtotime('-10 years'));?>"
              ...
              <div class="form-group ">
                <label for="Size">Ngày kết thúc: </label>
                <input name="date-e-e" id="date-e-e" type="date" class="form-control"  min="<?php echo date('Y-m-d', strtotime('-10 years'));?>"
                value="<?php echo date("Y-m-d");?>"
                max="<?php echo date("Y-m-d");?>"/>
...
<script >
  ...
  $(document).ready(function()
  {
   ...
   {
      alert("Ngày bắt đầu bị bỏ  trống");  
   }
   else if($('#date-e').val()=="")
   {
      alert("Ngày kết thúc bị bỏ  trống");  
   }
   else if($('#date-s').val() > $('#date-e').val())
   {
      alert("Ngày kết thúc bị lỗi");
  ...
  $(document).on('click', '.success', function(){
   ...
   $(document).ready(function()
   {
     ...
     else if($('#date-s-e').val() > $('#date-e-e').val())
   {
      alert("Ngày kết thúc bị lỗi");
       alert("Ngày kết thúc phải lớn  hơn ngày bắt đầu");    
   }
